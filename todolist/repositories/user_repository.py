    ...
    def get_full_user_info(self, msg_user_id: Union[str, int], messenger_id: int) -> sqlalchemy.engine.result.RowProxy:
        statement = sqlalchemy.sql.text(
            """select mu.*
                ...
                where mu.ext_id = :request_user_id and mu.messenger_type_id = :messenger_id""")

        return self.db.execute(
            statement, {'request_user_id': str(msg_user_id), 'messenger_id': str(messenger_id)}).fetchone()

    def get_active_users(self, messenger_id) -> List[sqlalchemy.engine.result.RowProxy]:
        statement = sqlalchemy.sql.text(
            """select mu.ext_id, u.id as request_user_id
                from public.msg_user mu
                left join dialog d on d.request_user_id = mu.id
                left join public.user u on d.request_user_id = u.id
    ...
    def get_active_user_messengers_by_msg_id(self, msg_user_id: str) -> List[Tuple[str, int]]:
        ...
        statement = sqlalchemy.sql.text(
            f'''select m.name, u_m.id
                ...
                join msg_user as ms_u on u_m.request_user_id = ms_u.id
                join messenger as m on m.id = ms_u.messenger_id
                where u_m.is_active = true
                    and u_m.request_user_id in (select u_m.request_user_id
                                        from msg_user as ms_u
                                        join user_messenger as u_m on ms_u.id = u_m.request_user_id
